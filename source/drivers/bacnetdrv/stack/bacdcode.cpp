...
/* from clause 20.2.1.3.2 Constructed Data 普通tag：前4位表示类型，后3位表示tag值*/
/* returns the number of apdu bytes consumed */
    ...
    uint32_t * value)
{
    ...
    if (IS_EXTENDED_VALUE(apdu[0])) {
        ...
        *value = 0;
        ...
        /* small value */
        *value = apdu[0] & 0x07; // 取后三位表示tagvalue
    }
    ...
    uint8_t tag_number)
{
    ...
    decode_tag_number(apdu, &my_tag_number); // tag号取第0个或第0个和第一个字节。如果byte0的高4位为1取第二个字节（扩展tag），否则取低四位作为tag值
    return (bool) (IS_CONTEXT_SPECIFIC(*apdu) &&
        (my_tag_number == tag_number)); // 扩展tag将返回false
}
    ...
    uint32_t * instance)
{
    ...
    *object_type =
        (uint16_t) (((value >> BACNET_INSTANCE_BITS) & BACNET_MAX_OBJECT)); // 第1个字节开始的4个字节，前10位表示对象类型
    *instance = (value & BACNET_MAX_INSTANCE); // 后10位表示实例号。中间12位预留？

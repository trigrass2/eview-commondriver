...
typedef struct _DRVGROUP
{
	...
	char			szHWBlockName[PK_DATABLOCKTYPE_MAXLEN + 1]; // DBn,I,Q

	int				nStartAddrBit;	// 起始位数，相对于整个物理块（AO、DBn、三菱的Q等）起始地址
	...
	int				nEndRegister;	// 起始寄存器号，是第1个tag点地址的寄存器，如第一个tag地址D1就是数字1。通常用nStartAddrBit可以计算出来，要考虑起始寄存器为1还是0开始？，按照nRegisterLenBits为单位计算
...
public:
	...
	void AutoGroupTags(vector<DRVGROUP *> &vecTagGrpInfo, GROUP_OPTION *pGroupOption);
	void GroupTagsOfGroupType(string strGrpType, vector<PKTAG *> &vecTagsOfGrpType, vector<DRVGROUP *> &vecTagGrpInfo, GROUP_OPTION *pGroupOption);
...
extern long TagsToGroups(vector<PKTAG *> &vecTags, GROUP_OPTION *pGroupOption, vector<DRVGROUP *> &vecTagGroup);

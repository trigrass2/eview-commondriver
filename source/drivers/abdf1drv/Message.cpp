...
int CMessage::BuildFinsBody_WriteMessage(char *szBodyBuffer, int nBufferLen, PKTAG *pTag, char *szBinValue, int nBinValueLen)
{
	...
	int nStartBits = pTag->nData1 + m_pTagGroup->nStartAddrBit;// 相对于块内的起始地址位（如AI/DI)+块的起始地址位 ？？？
	int nEndBits = pTag->nData2 + m_pTagGroup->nStartAddrBit; // 相对于块内的结束地址位（如AI/DI)+块的起始地址位 ？？？
...
long CMessage::BuildReadMessage(DRVGROUP *pTagGroup)
{
	...
	BuildFinsBody_ReadMessage(pMsgBuf, 12);  // 8个字节
	pMsgBuf += 12;
	count += 12;

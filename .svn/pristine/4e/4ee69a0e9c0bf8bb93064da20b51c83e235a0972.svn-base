...
typedef struct {
    uc P;	/* allways 0x32 */
	uc type;	/* Header type, one of 1,2,3 or 7. type 2 and 3 headers are two bytes longer. OxO1 为发送命令帧0x03 为应答帧,错误时返回的是0x02 */
	uc a,b;	/* currently unknown. Maybe it can be used for long numbers? */
	us number;	/* A number. This can be used to make sure a received answer 请求 、 应答事务标识 ， 应答帧直接复制返 回该部分*/
		/* corresponds to the request with the same number. */
	us plen;	/* length of parameters which follow this header 命令长度*/
...
EXPORTSPEC int DECL2 _daveSendRequestData(daveConnection * dc, int alt);
...
EXPORTSPEC int DECL2 _daveSendLengthAndIt(daveInterface * di, int len, uc *szContent);
...
//#ifndef _WIN32  //We can use this under windows only, but avoid error messages
//#define HANDLE int    
//#endif
...
extern int DECL2 _daveTimedRecv(daveInterface * di, uc *b, int len);

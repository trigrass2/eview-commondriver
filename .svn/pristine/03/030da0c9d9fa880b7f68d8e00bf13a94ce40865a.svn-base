    ...
    uint32_t * object_instance)
{
    ...
    if (apdu && apdu_len && object_type && object_instance) {
        /* Tag 0: objectIdentifier */
        if (!decode_is_context_tag(&apdu[len++], 0)) // 检查tag是否是0
            return -1;
    ...
    uint32_t * array_index)
{
    ...
    if (apdu && apdu_len && object_property && array_index) {
        /* Tag 2: propertyIdentifier */
        if (!IS_CONTEXT_SPECIFIC(apdu[len])) // 第三位
            return -1;
        len +=
            decode_tag_number_and_value(&apdu[len], &tag_number,
            &len_value_type);
        if (tag_number != 2) // 2表示是属性类型，0是objectID
            return -1;
        len += decode_enumerated(&apdu[len], len_value_type, &property);
        if (object_property)
        ...
        if ((len < apdu_len) && IS_CONTEXT_SPECIFIC(apdu[len]) &&
            ...
            tag_len =
                (unsigned) decode_tag_number_and_value(&apdu[len], &tag_number,
                &len_value_type);// tagnumber==4，表示取得是属性类型及索引
            if (tag_number == 3) { // 3表示数组
                len += tag_len;
                len +=
                    decode_unsigned(&apdu[len], len_value_type, &array_value);

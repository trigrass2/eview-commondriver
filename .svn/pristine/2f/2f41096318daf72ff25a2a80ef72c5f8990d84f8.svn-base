...
int DECL2 _daveConnectPLCTCP(daveConnection * dc) {
	...
	if (dc->iface->protocol == daveProtoISOTCP243) { // 243协议
		memcpy(dc->msgOut + 4, b243, sizeof(b243));
	}
	else if (dc->iface->protocol == daveProtoISOTCP) {// USI on TCP 协议
		if (!dc->routing) {
			memcpy(dc->msgOut + 4, b4, sizeof(b4)); // 18byte
			dc->msgOut[17] = dc->communicationType;  // (1=PG Communication,2=OP Communication,3=Step7Basic Communication),C1/C2参数

...
void CAutoGroupBlkDev::AutoGroupTags(vector<DRVGROUP *> &vecTagGrpInfo, GROUP_OPTION *pGroupOption)
{
	// 先按照点的类型分成不同的组
...
// GroupType. for modbus: ai/ao/di/do    for simens: db5/m/i....
void CAutoGroupBlkDev::GroupTagsOfGroupType(string strGrpType, vector<PKTAG *> & vecTags, vector<DRVGROUP *> &vecGroups, GROUP_OPTION *pGroupOption)
{
	int nIsGroupTag = 1;
...
// 会修改pDevTags的值
long TagsToGroups(vector<PKTAG *> &vecAllDevTags, GROUP_OPTION *pGroupOption, vector<DRVGROUP *> &vecTagGroup)
{

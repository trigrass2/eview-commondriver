...
int main(int argc, char **argv) {
	...
	dc =daveNewConnection(di, plcPPI, 11, 22);  // 1 or 0 is OK .对于PPI来说，调用不调用daveConnectPLC都没关系
	int nRet = 0;
	//nRet = daveConnectPLC(dc);
//
...
*/	    
    ...
    res=daveReadBytes(dc,daveDB,1,1,2,NULL);
	if (res==0) {
	    a=daveGetU16(dc);
	    printf("VW0: %d\n",a);
	    a=daveGetU16(dc);
...
*/	    
	...
	{
		printf("read failed, return:%d", res);
	}

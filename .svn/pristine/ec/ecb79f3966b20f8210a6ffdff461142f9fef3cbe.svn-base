    ...
    uint16_t bbmd_port)
{
    ...
    if (mtu_len > 0) {
        ...
        rv = bvlc_send_mpdu(&bbmd, &mtu[0], mtu_len,NULL);
    }
...
{       /* length of the NPDU  */
    ...
    for (i = 0; i < MAX_BBMD_ENTRIES; i++) {
        if (BBMD_Table[i].valid) {
            ...
            bvlc_send_mpdu(&bip_dest, mtu, mtu_len,NULL);
    ...
    BACNET_BVLC_RESULT result_code)
{
    ...
    bvlc_send_mpdu(dest, mtu, mtu_len,NULL);
    ...
    struct sockaddr_in *dest)
{
    ...
    if (mtu_len) {
        bvlc_send_mpdu(dest, &mtu[0], mtu_len,NULL);
    }
    ...
    switch (BVLC_Function_Code) {
        ...
        case BVLC_FORWARDED_NPDU:
            ...
            if (bvlc_bdt_member_mask_is_unicast(&sin)) {
                ...
                bvlc_send_mpdu(&dest, &npdu[4 + 6], npdu_len,NULL);
            }
...
{       /* number of bytes of data */
	...
	return bvlc_send_mpdu(&bvlc_dest, mtu, mtu_len,hDevice);
}
#endif
    ...
    uint16_t time_to_live_seconds)
{
    ...
    retval = bvlc_send_mpdu(&Remote_BBMD, &mtu[0], mtu_len,NULL);
    return retval;
}

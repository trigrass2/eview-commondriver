...
int CMqttImpl::mqttClient_Init(void* pObj)
{
	...
	if (nRet != 0)
	{
		Drv_LogMessage(PK_LOGLEVEL_ERROR, "mosquitto_lib_init failed, ret:%d", nRet);
		return nRet;
	}
	...
	if (m_hMqtt == NULL)
	{
		Drv_LogMessage(PK_LOGLEVEL_ERROR, "mosquitto_new failed, retvalue == NULL, ret:%d", MQTT_NEW_HANDLE_ERROR);
...
int CMqttImpl::mqttClient_Pub(const char* topic, int *mid, int msgLen, const char* szMsg)
{
	if (m_bInit == false)
	{
		Drv_LogMessage(PK_LOGLEVEL_ERROR, "mqttClient_Pub failed, please init first");
		return MQTT_NO_INITED;
	}

	if (m_bConnected == false)
	{
		Drv_LogMessage(PK_LOGLEVEL_ERROR, "mqttClient_Pub failed, please connect first");
	...
	if (nRet != 0)
	{
		Drv_LogMessage(PK_LOGLEVEL_ERROR, "mosquitto_publish failed, return:%d", nRet);
		return MQTT_NO_CONNECTED;
	}
...
int CMqttImpl::mqttClient_Connect(int nKeepAlive, CBFuncConnect* CBConn, CBFuncDisConnect* CBDisConn, CBFuncPublish* CBPub, CBFuncSubscribe* CBSub)
{
	if (m_bInit == false)
	{
		Drv_LogMessage(PK_LOGLEVEL_ERROR, "mqttClient_Connect failed, please init first");
		return MQTT_NO_INITED;
	}
	...
	{
		Drv_LogMessage(PK_LOGLEVEL_INFO, "mosquitto_connect success, url:%s:%d, keepalive:%d", m_szAddrIP, m_nPort, nKeepAlive);
		m_bConnected = true;
	}
	else
	{
		Drv_LogMessage(PK_LOGLEVEL_ERROR, "mosquitto_connect failed, return:%d, url:%s:%d, keepalive:%d", nRet, m_szAddrIP, m_nPort, nKeepAlive);
		m_bConnected = false;
	}
	return nRet;
}
...
int CMqttImpl::mqttClient_mSub(vector<string> vecTopics, vector<string>& vecSucceed, int *mid)
{
	...
	for (int i = 0; i < vecTopics.size(); i++)
	{
		...
		if (nRet == MOSQ_ERR_SUCCESS)
		{
			Drv_LogMessage(PK_LOGLEVEL_INFO, "mosquitto_subscribe success, channel:%s", strTopic.c_str());
			m_bConnected = true;
		}
		else
		{
			Drv_LogMessage(PK_LOGLEVEL_ERROR, "mosquitto_subscribe failed, return:%d, channel:%s", nRet, strTopic.c_str());
			m_bConnected = false;
		}
...
int CMqttImpl::mqttClient_Sub(const char* topic, int *mid)
{
	...
	if (m_bConnected == false)
	{
		Drv_LogMessage(PK_LOGLEVEL_ERROR, "连接mqttserver失败，未连接");
	}
